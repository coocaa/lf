...
1. 测试数据隔离, 实现数据驱动
2. 测试数据随机化: 支持模板语法, 用例中可以定义变量随机生成数字, 电话, 地区, 邮箱, 姓名, 公司名等等...
3. 支持多接口数据依赖: 如A接口需要同时依赖B、C接口的响应数据作为参数
4. 数据库断言: 直接在测试用例中写入查询的sql即可断言，无需编写代码
5. 响应内容断言: 可使用jsonpath表达式, 提取接口响应内容进行断言
6. 动态多断言: 如接口需要同时校验响应数据和sql校验，支持多场景断言
7. 自动生成用例代码: 测试人员在yaml文件中填写好测试用例, 程序可以直接生成用例代码，纯小白也能使用
8. 测试报告展示历史测试结果, 表格形式和曲线图显示分别展示
9. 统计接口的运行时长: 拓展功能，订制开关可以决定是否需要使用
10. 日志输出: 记录每个接口的详细信息，包括断言方式以及预期结果, 同时订制了开关可以决定是否需要打印日志
11. 钉钉、企业微信, 飞书, 邮件通知: 支持多种通知场景
12. 多线程执行, 失败用例重跑机制, 自定义跳过用例等等
